...
public class BOJ25514 {
    ...
    public static Node run(int dy, int dx, Node curr) {
        int nxtX = curr.x + dx;
        int nxtY = curr.y + dy;

        while (true) {
            if (!canGo(nxtY, nxtX)) {
                return new Node(nxtY - dy, nxtX - dx, curr.dist + 1);
            } else {
                if (map[nxtY][nxtX] == 7) {
                    return new Node(nxtY, nxtX, curr.dist + 1);
                }
            }
        ...
        for (int floor = 0; floor < DEST_NUM; floor++) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (int i = 0; i < dirX.length; i++) {
                    Node nxt = run(dirY[i], dirX[i], curr);
                    if (!visited[floor][nxt.y][nxt.x]) {
                        queue.add(nxt);
                        visited[floor][nxt.y][nxt.x] = true;
                    }
                }
